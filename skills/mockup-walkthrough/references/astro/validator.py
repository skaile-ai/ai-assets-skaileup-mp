...
# the spec panel's own `<h3>` skeleton and MUST NEVER be rendered as an
# annotatable `el-region` widget, i.e. their slug MUST NEVER appear as a
...
# <table>'s <tbody>"). Stdlib-only, ported from
# mockup-walkthrough-static-html's validator.py (same shape, same style).
...
def find_descendant_or_self_anchor(node: DomNode) -> DomNode | None:
    ...
    `<a>`). `items[]` entries need the opposite direction: a list `<li>`
    (or nav/tabs entry) carries its own `data-spec-element`, and
    `items[].target` wraps that entry's content in `<a>` — i.e. the link is
    a *child* of the entry node, not an ancestor of it.
    """
...
def resolve_href(site: Path, href: str) -> Path:
    """Resolve an Astro root-relative href to the built file it names.

    Astro's routes are served as clean, extension-less, root-relative URLs
    (`/screen/00_auth/login`, `/journey/sign-in`, `/`) — NOT static-html's
    page-relative `../<group>/<name>.html` scheme (see this renderer's
    SKILL.md § Renderer Contract for why: the built `.html` files exist on
    disk, but the href the template emits is the clean-URL form). Resolving
    a href to a filesystem path is therefore site-root-relative, not
    page-relative, and needs no `page_path` argument at all.
    """
    href = href.split("#", 1)[0]
    path = href.lstrip("/")
    if path == "":
...
def has_navigable_anchor(nav: DomNode) -> bool:
    """True if `nav` contains at least one `<a>` with a real (non-`#`,
    non-empty) href. Astro's hrefs are intentionally root-relative
    (`/screen/...`) — unlike static-html's `has_relative_anchor`, a leading
    `/` here is normal and expected, not a violation.
    """
...
# resolution, § App-shell navigation) — ported from
# mockup-walkthrough-static-html's validator.py, `resolve_href` swapped for
# astro's root-relative scheme (see `resolve_href` above). ─────────────
    ...
    *,
    ...
    if resolves:
        ...
        if href == "#":
            ...
            return
        resolved_path = resolve_href(site, href)
...
def check_targets(site: Path, manifest: dict, report: Report) -> None:
    ...
    for screen in screens:
        ...
        # class="app-nav"> with at least one navigable <a>.
        app_navs = find_all(
            dom,
            lambda n: n.tag == "nav" and "app-nav" in n.attrs.get("class", "").split(),
        )
        if not app_navs or not any(has_navigable_anchor(n) for n in app_navs):
            report.add(
                str(rendered_path),
                'expected >= 1 <nav class="app-nav"> containing a '
                "navigable <a> (§ App-shell navigation)",
            )
    ...
    for i, entry in enumerate(manifest.get("app_nav", [])):
        ...
        resolved_path = resolve_href(site, target)
        if not resolved_path.exists():
            report.add(
...
# ported from mockup-walkthrough-static-html's validator.py verbatim (astro
# produces the same static HTML shape in the end, so the checks translate
# unchanged). ──────────────────────────────────────────────────────────
...
def check_content_fidelity(site: Path, manifest: dict, report: Report) -> None:
    ...
    for screen in manifest.get("screens", []):
        ...
        spec_panels = [
            (t, a) for t, a in tags if t == "details" and "spec-panel" in a.get("class", "").split()
        ]
